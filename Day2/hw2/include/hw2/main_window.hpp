...
private slots:
    // 과제 2 - 경로 기록/재생 버튼 슬롯 (Qt 자동 슬롯 연결 규칙: on_<objectName>_<signal>)
...
private:
    ...
    void startRecording();
    void stopRecording();
    void sampleRecordedPose();
    void startReplay();
    void stopReplay();
    void replayNextStep();
    void updateRecordStatusLabel();

    bool isRecording_ = false;
    bool isReplaying_ = false;
    QVector<RecordedPose> recordedPath_;
    int replayIndex_ = 0;
    double recordedDistance_ = 0.0;
    double recordElapsedSec_ = 0.0;
    double lastRecordX_ = 0.0;
    double lastRecordY_ = 0.0;
