...
namespace
{
constexpr int kRecordIntervalMs = 100;  // 경로 기록/재생 샘플링 주기 (ms)
}  // namespace
...
{
    // 재생 중이거나 이미 기록 중이면 무시
    if (isRecording_ || isReplaying_)
    {
        return;
    }

    recordedPath_.clear();
    recordedDistance_ = 0.0;
    recordElapsedSec_ = 0.0;
    lastRecordX_ = curX_;
    lastRecordY_ = curY_;
    isRecording_ = true;

    // 기록 시작 시점의 pose도 경로에 포함시켜 재생 시 시작 위치가 어긋나지 않도록 함
    recordedPath_.push_back({curX_, curY_, curTheta_});

    if (!recordTimer_)
    {
        recordTimer_ = new QTimer(this);
        connect(recordTimer_, &QTimer::timeout, this, &MainWindow::sampleRecordedPose);
    }
    recordTimer_->start(kRecordIntervalMs);

    updateRecordStatusLabel();
}
...
void MainWindow::sampleRecordedPose()
{
    if (!isRecording_)
    {
        return;
    ...
    const double dx = curX_ - lastRecordX_;
    const double dy = curY_ - lastRecordY_;
    recordedDistance_ += std::sqrt(dx * dx + dy * dy);
    lastRecordX_ = curX_;
    lastRecordY_ = curY_;

    recordElapsedSec_ += kRecordIntervalMs / 1000.0;
    recordedPath_.push_back({curX_, curY_, curTheta_});
...
{
    // 기록 중이거나 이미 재생 중이거나 기록된 경로가 없으면 무시
    if (isRecording_ || isReplaying_ || recordedPath_.isEmpty())
    {
        return;
    }
    ...
    if (!replayTimer_)
    {
        ...
        connect(replayTimer_, &QTimer::timeout, this, &MainWindow::replayNextStep);
    }
    // 기록할 때와 동일한 주기로 저장된 pose를 순서대로 재현
    replayTimer_->start(kRecordIntervalMs);

    updateRecordStatusLabel();
}

void MainWindow::replayNextStep()
{
    if (replayIndex_ >= recordedPath_.size())
    {
        stopReplay();
        return;
    }

    const RecordedPose& pose = recordedPath_[replayIndex_];
    qnode->teleportAbsolute(pose.x, pose.y, pose.theta);
    ++replayIndex_;
...
void MainWindow::updateRecordStatusLabel()
{
    QString state;
    if (isRecording_)
    {
    ...
    else if (isReplaying_)
    {
        state = QString("재생 중 (%1 / %2)").arg(replayIndex_).arg(recordedPath_.size());
    }
    else
    {
        state = "대기";
    ...
    ui->recordStatusLabel->setText(
        QString("상태: %1\n총 이동 거리: %2 m\n소요 시간: %3 s")
            .arg(state)
            .arg(recordedDistance_, 0, 'f', 2)
            .arg(recordElapsedSec_, 0, 'f', 1));
}